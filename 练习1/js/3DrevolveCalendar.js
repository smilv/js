;(function () {
    var Calendar = function () {     //创建一个Calendar对象
    ...
    p.revolveReverse = function () {
        ...
        doKeyframes(fe, "flipOutY", "1s", function () {
            fe.style.display = "none";
            re.style.display = "block";
            doKeyframes(re, "flipOutY2", "1s", function () {
    ...
    p.calendarObverse = function () {
        var fe = document.querySelector(".front");
        var re = document.querySelector(".reverseSide");
        doKeyframes(re, "flipOutY", "1s", function () {
            re.style.display = "none";
            fe.style.display = "block";
            doKeyframes(fe, "flipOutY2", "1s", function () {
    ...
    p.init = function () {
        ...
        for (var i = 0; i < trs; i++) {
            ...
            for (var j = 0; j < 7; j++) {
                if (i === 0 && j < weeks) {
                    td += '<div class="calendar_dates" style="width:20px;height:19px;float:left;"><span></span></div>'
                } else {
                    if (n <= days) {
                        td += '<div class="calendar_dates" style="width:20px;height:19px;float:left;"><span>' + n++ + '</span></div>'
                    }
                }
            }
    ...
    p.curDate = function () {
        ...
        for (var i = 0; i < days; i++) {
                if ($('.calendar_date div')[i].innerHTML == '<span>'+this.date+'</span>') {
                    $('.calendar_date div')[i].style.border = '1px solid #fcefa1';
                    $('.calendar_date div')[i].style.background = '#fcfaf1';
                }
        }
